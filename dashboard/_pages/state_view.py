...
def render(selected_year: int | None, selected_disease: str) -> None:
    ...
    with col_left:
        _render_state_trend(selected_state, disease_filter, selected_year, state_df)
...
def _render_state_kpis(df: pd.DataFrame, state: str) -> None:
    ...
    # Use peak (max) weekly incidence — mean is diluted by many zero-case weeks
    # and rounds to "0.0" even for states with genuine burden.
    peak_incidence = None
    if "incidence_per_100k" in df.columns:
        nonzero = df["incidence_per_100k"].replace(0, pd.NA).dropna()
        peak_incidence = float(nonzero.max()) if not nonzero.empty else None
    ...
    with col4:
        st.metric("Peak Incidence /100k",
                  f"{peak_incidence:.3f}" if peak_incidence is not None else "N/A")
...
def _render_state_trend(
    ...
    """Monthly trend for the selected state — all diseases or a single one."""
    st.subheader("📈 Case Trend")

    if disease is None:
        # All diseases — build from already-loaded state_df
        if "report_date" not in state_df.columns or state_df.empty:
        ...
        df["period"] = df["report_date"].dt.to_period("M").astype(str)
        monthly = (
            df.groupby(["period", "disease"])["confirmed_cases"]
            .sum()
            .reset_index()
        )
        if monthly.empty:
        ...
        fig = px.line(
            ...
            y        = "confirmed_cases",
            color    = "disease",
            title    = f"All Diseases — {state}",
            labels   = {"period": "Month", "confirmed_cases": "Cases", "disease": "Disease"},
            template = "plotly_white",
        )
    else:
        with st.spinner("Loading trend..."):
            trend_df = get_trend(disease=disease, state=state, freq="monthly")
        if trend_df.empty:
            st.info("No trend data available for this state.")
            return
...
def _render_cfr_benchmark(
    ...
    _DISEASES = ["Cholera", "Lassa Fever", "Meningitis", "Mpox", "Yellow Fever"]

    with st.spinner("Loading CFR benchmarks..."):
        if disease:
            cfr_df = get_cfr_benchmark(disease=disease, year=year)
            if not cfr_df.empty:
                cfr_df["disease"] = disease
        else:
            frames = []
            for d in _DISEASES:
                df = get_cfr_benchmark(disease=d, year=year)
                if not df.empty:
                    df["disease"] = d
                    frames.append(df)
            cfr_df = pd.concat(frames, ignore_index=True) if frames else pd.DataFrame()
    ...
    query_disease = disease or "All Diseases"
    color_col = "disease" if not disease else "flag"
    color_map = {"HIGH": "#E24B4A", "NORMAL": "#1D9E75", "LOW": "#85B7EB"}
    ...
    fig = px.bar(
        ...
        y      = "avg_cfr",
        color  = color_col,
        color_discrete_map = color_map if disease else None,
        barmode = "group" if not disease else "relative",
        title    = f"CFR by state — {query_disease}",
        labels   = {"avg_cfr": "Avg CFR (%)", "state": "State", "disease": "Disease"},
        template = "plotly_white",
    )
