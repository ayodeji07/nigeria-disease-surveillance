...
def _render_trend_chart(year: int | None, disease: str) -> None:
    ...
    with st.spinner("Loading trend data..."):
        if disease_filter:
            trend_df = get_trend(disease=disease_filter, freq="monthly")
            if not trend_df.empty:
                trend_df["disease"] = disease_filter
        else:
            frames = []
            for d in _ALL_DISEASES:
                df = get_trend(disease=d, freq="monthly")
                if not df.empty:
                    df["disease"] = d
                    frames.append(df)
            trend_df = pd.concat(frames, ignore_index=True) if frames else pd.DataFrame()
    ...
    fig = px.line(
        ...
        x      = "period",
        y      = "confirmed_cases",
        color  = "disease" if disease_filter is None else None,
        title  = f"Monthly confirmed cases — {disease_filter or 'All Diseases'}",
        labels = {"period": "Month", "confirmed_cases": "Confirmed Cases", "disease": "Disease"},
...
def _render_cfr_trend(disease: str) -> None:
    ...
    fig = px.line(
        ...
        y     = "cfr_pct",
        color = "disease" if disease_filter is None else None,
        title = f"Monthly CFR — {disease_filter or 'All Diseases'}",
        labels = {"period": "Month", "cfr_pct": "CFR (%)", "disease": "Disease"},
        template = "plotly_white",
    )
    if disease_filter:
...
def _render_outbreak_alerts(disease: str, year: int | None) -> None:
    ...
    disease_filter = None if disease == "All diseases" else disease
    query_disease  = disease_filter or "Lassa Fever"
