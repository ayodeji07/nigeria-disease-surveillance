...
def render(selected_year: Optional[int], selected_disease: str) -> None:
    ...
    query_disease = selected_disease if selected_disease != "All diseases" else "Cholera"
...
def _render_trend_tab(disease: str, year: Optional[int]) -> None:
    ...
    st.markdown(
        f"""
        <div style="padding:1rem;border-radius:8px;border:1px solid #ddd;">
            <p style="font-size:22px;font-weight:500;color:{colour};margin:0">
                {trend.title()} {"✓" if sig else "(not significant)"}
            </p>
